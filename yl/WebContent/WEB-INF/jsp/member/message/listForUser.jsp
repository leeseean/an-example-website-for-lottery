				...
				<div class="page-body switch-mod">
					<div class="switch-menu s16">
						<ul class="center clear">
							...
							<li class="item"
								onclick="goParent('${ctx}/message/listForSys?id=${item.id}')">系统信息</li>
						</ul>
					</div>
										...
										<c:forEach var="item" items="${page.result}" varStatus="s">
											<tr class="sheet-body s12 gray-dark">
												<td align="center"><fmt:formatDate
														value="${item.create_time}" pattern="yyyy-MM-dd HH:mm" />
												</td>

												<td align="center"><a
													href="${ctx}/message/showMessage?id=${item.id}"> <font
														color='${item.read_status==1}?"blue":"red"'>${item.msg_title}
													</font> </a></td>

												<td align="center">${item.read_status==1?"已读":"未读"}</td>
												<td align="center"><input type="button"
													onclick="window.location.href='${ctx}/message/showMessage?id=${item.id}'"
													name="b_tn" value="查看" style="width: 50px;height: 25px"></input>
													<input type="button"
													onclick="window.location.href='${ctx}/message/deleteMessage?id=${item.id}'"
													name="b_tn" value="删除" style="width: 50px;height: 25px"></input>
												</td>
											</tr>
										</c:forEach>
										<c:if test="${!empty page.result}">
