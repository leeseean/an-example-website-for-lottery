		...
		<tbody>
			...
			<c:forEach var="item" items="${dataList}">
				...
				<tr id="TR_${item.id }" style="display: ;">
					...
					<td class="b_1stR">${item.hratioO ==null ?'':'大'}${item.hratioO } <a href="javascript://" onClick="parent.orderFrame.betOrder('${item.gid}','${timeType}','FT','r','dx','dx','H','h')" title="大"> ${item.iorHouc } </a></td>
				</tr>
				<tr id="TR1_${item.id }">
					...
					<td class="b_1stR">${item.hratioU ==null ?'':'小'}${item.hratioU } <a href="javascript://" onClick="parent.orderFrame.betOrder('${item.gid}','${timeType}','FT','r','dx','','C','h')" title="小"> ${item.iorHouh } </a></td>
				</tr>
				<tr id="TR2_${item.id }">
					<td class="drawn_td">
