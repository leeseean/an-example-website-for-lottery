				...
				<c:forEach var="item" items="${dataList}">
					...
					<tr id="TR_${item.id }" style="display: ;">
						...
						<c:if test="${item.setType eq '1'}">
							...
							<td class="b_1stR">${item.hratioO ==null ?'':'大'}${item.hratioO } <a href="javascript://" onClick="parent.orderFrame.betOrder('${item.gid}','${timeType}','FT','re','dx','dx','H','h')" title="${item.teamH }"   <c:if test="${fn:contains(item.frasherDate, 'iorHouc')}">style="background-color : yellow"</c:if>> ${item.iorHouc } </a></td>
						</c:if>
					...
					<tr id="TR1_${item.id }" style="display: ;">
						...
						<c:if test="${item.setType eq '1' }">
							...
							<td class="b_1stR">${item.hratioU ==null ?'':'小'}${item.hratioU } <a href="javascript://" onClick="parent.orderFrame.betOrder('${item.gid}','${timeType}','FT','re','dx','dx','C','h')" title="${item.teamH }"  <c:if test="${fn:contains(item.frasherDate, 'iorHouh')}">style="background-color : yellow"</c:if>> ${item.iorHouh } </a></td>
						</c:if>
						<c:if test="${item.setType eq '2' or item.setType eq '3' }">
							<td class="b_1st">&nbsp;</td>
							<td class="b_1stR">&nbsp;</td>
