	...
	<c:if test="${!empty bets}">
		...
		<c:forEach var="bet" items="${bets}">
			...
			<c:if test="${bet.matchRtype eq 'p3'}">
		        ...
		        <td>
					${detail.betOddsDes }<br><font color="green">${fn:length(bet.details)}串一</font><br>
			        <font color="#0000CC">${bet.betWagersId}</font>
			        <br>
					</td>
					<td>
					<c:forEach var="detail" items="${bet.details}">
					 	${detail.league }<br>${detail.teamH }   <font color="#0000BB"><b>.VS.</b></font> ${detail.teamC }   <br><font color="#CC0000">${detail.betOddsName } </font> @ <font color="#CC0000"><b>${detail.betOdds }</b></font>
