angular.module('AppController')
	.controller('CpFTCController', ['$rootScope', '$scope', '$stateParams' , '$state' , 
		...
		var code2Ids = {
				...
				ez: {id: 16, name: '二字'},
				sz: {id: 17, name: '三字'},
				yzgg: {id: 18, name: '一字过关'},
				kd: {id: 19, name: '跨度'},
				zxs: {id: 20, name: '组选三'},
				zxl: {id: 21, name: '组选六'},
				fszh: {id: 22, name: '复式组合'},
				qt: {id: 23, name: '其他'},
		};
		...
		if ($scope.code2 == 'yzgg') {
			...
			$scope.confirm = function (cb) {
				...
				for (var key in $scope.selectedPeiyu) {
					...
					selectedPeiyu[key] = $scope.selectedPeiyu[key];
				}
				if (Object.keys(selectedPeiyu).length < 2) {
					ngDialog.open({
			...
			$scope.confirm = function () {
				...
				for (var number in $scope.selectedNumber) {
					if ($scope.selectedNumber[number] == false) continue;
					...
					CaipiaoMultiGroupPreorder.ZXS($scope.code.toUpperCase(), $scope.code2.toUpperCase(), $scope.weizhi.toUpperCase(), 3, numbers.join(','))
						.then(function (res) {
							if (typeof res['data'] == 'object') {
								...
								CaipiaoCreateOrder.resetItems();
								for (var index in res['data']['orderList']) {
									CaipiaoCreateOrder.setOrderFlag('group').setItem({
										'preorder': res['data']['orderList'][index],
										'xzje': $rootScope.xzje,
										'token': token
									});
