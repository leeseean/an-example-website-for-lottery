      	...
      	<c:forEach var="item" items="${map.value }">
	        <div data-role="collapsible" data-collapsed="false" data-mini="true" data-theme="b" data-content-theme="d" class="ui-mini f12">
	          ...
	          <ul data-role="listview">
	          	...
	          	<li>局号：${item.betGameContent}</li>
	            <li>投注金额：${item.betIn}￥</li>
       ...
       <div data-role="navbar">
         <ul>
           <c:forEach var="page" items="${pageList }" varStatus="status">
             <li>
             	<a href="#${page }" data-transition="none" class='<c:if test="${status.index == mapStatus.index }">ui-btn-active ui-state-persist</c:if>' data-role="button">${status.index + 1}</a>
             </li>
           </c:forEach>
         </ul>
       </div>
