 ...
 *
 */
@Service("ntInterfaceServiceImpl")
public class NtInterfaceServiceImpl extends BaseFlatInfo implements FlatInterfaceService {
	...
	public String login(HttpServletRequest request) {
		try {
			...
			String game = request.getParameter(NtConts.GAME);

			Map<String, String> params = new HashMap<String, String>();
			params.put(NtConts.USER_ID, webUserFlat.getNtUserName());// 游戏帐号
			params.put(NtConts.GAME, game);// 游戏名称
			NtResResult result = NTIfcUtil.login(params);
		...
		if(StringUtils.isEmpty(webUserFlat.getNtUserName())){
			 try {
				...
				if(webUserFlat.getNtStatus()!=null && webUserFlat.getNtStatus().intValue()==1){
					 ...
					 tsMsg  = updateNtAccountPonit(webUser, flatName, webUserFlat.getNtUserName(), optType, points, eduForward, eduForwardRemark);
				 }else{
	...
	private String updateNtAccountPonit(WebUser webUser,String flatName,String flatAccount,String optType
