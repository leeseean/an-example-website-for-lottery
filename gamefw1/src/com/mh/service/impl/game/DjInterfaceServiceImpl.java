 ...
 *
 */
@Service("djInterfaceServiceImpl")
public class DjInterfaceServiceImpl extends BaseFlatInfo implements FlatInterfaceService {
	...
	public String login(HttpServletRequest request) {
		try {
			...
			params.put(DjConts.PASSWORD, webUserFlat.getDjPassword());// 密码
			DjResResult result = DJIfcUtil.login(params);
			if (result != null) {
		...
		if(StringUtils.isEmpty(webUserFlat.getDjUserName())){
			 try {
				...
				if(webUserFlat.getDjStatus()!= null && webUserFlat.getDjStatus().intValue()==1){
					 ...
					 tsMsg  = updateDjAccountPonit(webUser, flatName, webUserFlat.getDjUserName(), optType, points, eduForward, eduForwardRemark);
				 }else{
	...
	private String updateDjAccountPonit(WebUser webUser,String flatName,String flatAccount,String optType
